    ...
    if($_SESSION['route'][2]=='0'){
        ...
        if($r->rowCount()>0){
            ...
            if($r->rowCount()>0){
                ...
                unset($pr[':id']);
                $sql="INSERT INTO administradores (nId, wNome, wLogin, sSenha) VALUES ('',:nome,:login,sha1(:senha))";
                $r=SqlQuery($sql,$pr);
                $_SESSION['view']='list';
                $_SESSION['erro_no']=1;
...
function administrador_permissoes(){
    ...
    foreach($pr[':ids'] as $l){
        $r=SqlQuery("INSERT into administrador_permissoes (nId, nId_administrador, nId_item) values ('','".$pr[':id_adm']."', '".$l[0]."')");
    }
    $_SESSION['erro_no']='1';
